import { envList, github } from '@/helpers/api/server'
...
export async function GET() {
	try {
		const topics = envList('GITHUB_REPO_TOPICS')
		...
		const filteredRepos = data
			.filter(repo => topics.some(topic => repo.topics.includes(topic)))
		...
		for (const repository of filteredRepos) {
			...
			const safeHomepage = sanitizeUrl(repository.homepage)
			if (safeHomepage) {
				badges.push({
					color: 'purple',
					text: 'Website',
			...
			response.push({
				...
				homepage: sanitizeUrl(repository.homepage),
				badges
			})
		}
